...
let usage = "real_sparrow_preanalysis_dump --module <file.c>... --out <dir>"
...
let write_module source =
  ...
  Sparrow_modular_ocaml.Real_sparrow_artifact.write_json path
    (Sparrow_modular_ocaml.Real_sparrow_preanalysis.artifact_for_module source);
  path

let () =
  Arg.parse
    [
      "--module", Arg.String add_module, "C source module to run through active real PreAnalysis";
  ...
  let manifest = `Assoc [
    ...
    "boundary", `String Sparrow_modular_ocaml.Real_sparrow_preanalysis.boundary;
  ] in
