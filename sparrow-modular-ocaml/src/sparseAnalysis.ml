...
let last_worklist_initialized = ref false
let last_finalize_performed = ref false
let last_widening_performed = ref false
let last_widening_iterations = ref 0
let last_widening_worklist_drained = ref false
let last_narrowing_enabled = ref false
let last_narrowing_applicable = ref false
let last_narrowing_performed = ref false
let last_narrowing_iterations = ref 0
let last_narrowing_worklist_drained = ref false

let reset_completion () =
  last_worklist_initialized := false;
  last_finalize_performed := false;
  last_widening_performed := false;
  last_widening_iterations := 0;
  last_widening_worklist_drained := false;
  last_narrowing_enabled := !Options.narrow;
  last_narrowing_applicable := !Options.narrow;
  last_narrowing_performed := false;
  last_narrowing_iterations := 0;
  last_narrowing_worklist_drained := false

let completion_json () =
  `Assoc [
    "worklist_initialized", `Bool !last_worklist_initialized;
    "finalize_performed", `Bool !last_finalize_performed;
    "widening_performed", `Bool !last_widening_performed;
    "widening_iterations", `Int !last_widening_iterations;
    "widening_worklist_drained", `Bool !last_widening_worklist_drained;
    "narrowing_enabled", `Bool !last_narrowing_enabled;
    "narrowing_applicable", `Bool !last_narrowing_applicable;
    "narrowing_performed", `Bool !last_narrowing_performed;
    "narrowing_iterations", `Int !last_narrowing_iterations;
    "narrowing_worklist_drained", `Bool !last_narrowing_worklist_drained;
    "worklist_drained", `Bool (!last_widening_worklist_drained && ((not !last_narrowing_applicable) || !last_narrowing_worklist_drained));
    "pfs", `Int !Options.pfs;
...
struct
  ...
  =fun spec dug (worklist, global, inputof, outputof) ->
    ...
    |> (fun ((works, _, _, _) as x) ->
        last_widening_performed := true;
        last_widening_iterations := !total_iterations;
        last_widening_worklist_drained := (match Worklist.pick works with None -> true | Some _ -> false);
  ...
  =fun spec dug (worklist, global, inputof, outputof) ->
    total_iterations := 0;
    last_narrowing_applicable := true;
    worklist
    ...
    |> (fun ((works, _, _, _) as x) ->
        last_narrowing_performed := true;
        last_narrowing_iterations := !total_iterations;
        last_narrowing_worklist_drained := (match Worklist.pick works with None -> true | Some _ -> false);
...
(*          ("dugraph-inter", DUGraph.to_json_inter dug access);*)
  ...
  =fun spec global ->
    ...
    last_worklist_initialized := true;
