...
let comma_join xs =
  ...
  in
  match xs with [] -> "" | x :: rest -> loop x rest
...
let bool_field name json =
  match assoc_field name json with Some (`Bool b) -> Some b | _ -> None
...
type declaration = { name : string; kind : string; source : string }
...
type phase_event = { phase_index : int; module_id : string; event : string }
...
let external_summary_v1_compat_to_json summary =
  `Assoc
    [
      ...
      ( "global_write_summary_placeholder",
        summary.global_write_summary_placeholder );
...
let external_summary_to_json summary =
  ...
  in
  let memory_delta_validation_input =
    `Assoc
      [
  ...
  in
  let memory_delta_validation =
    MemoryDelta.validate_summary_json memory_delta_validation_input
  in
  ...
  `Assoc
    [
      ...
      ( "typed_memory_delta_validation",
        MemoryDelta.validation_result_json memory_delta_validation );
      ("provenance", summary.provenance);
      ( "external_summary_v1_compat",
        external_summary_v1_compat_to_json summary.v1_compat );
    ]
...
let declaration_name_set decls =
  decls |> List.map (fun d -> d.name) |> sort_strings

let has_name names name = List.exists (( = ) name) names

let residual_api_model_supported name =
  List.mem name [ "memcpy"; "strcpy"; "strlen" ]
...
let declarations_from_global global =
  ...
  let imports =
    ...
    |> List.filter (fun d ->
           not (d.kind = "function" && residual_api_model_supported d.name))
  in
  (imports, exports)

let declarations_for_result result =
  declarations_from_global result.MetaSparse.before
...
let input_module_json bundle =
  `Assoc
    [
      ...
      ( "stage2_input_key",
        `String (bundle.result.module_id ^ ":" ^ bundle.result.source_hash) );
...
let make_bundle ~artifact_path result artifact =
  if not (validate_module_artifact artifact) then
    failwith
      ("module artifact is not a valid module-local pre-link AS artifact: "
     ^ artifact_path);
                       ...
                       |> List.map (fun export_decl ->
                              `Assoc
                                [
                                  ...
                                  ( "importer_module",
                                    `String importer.result.module_id );
                                  ( "exporter_module",
                                    `String exporter.result.module_id );
                                  ("import_source", `String import_decl.source);
                                  ("export_source", `String export_decl.source);
                                  ( "match_kind",
                                    `String
                                      "parsed-cil-declaration-to-definition" );
                                ]))))
  |> sort_json
...
let module_key bundle =
  bundle.result.module_id ^ ":" ^ bundle.result.source_hash
...
let row_memory row =
  match assoc_field "memory" row with Some (`List memory) -> memory | _ -> []

let cell_location cell =
  match string_field "location" cell with Some s -> s | None -> ""

let cell_value cell =
  match string_field "value" cell with Some s -> s | None -> ""
...
let effect_location_kind location =
  if is_parenthesized_location location && contains location "," then
...
let typed_effect ~domain ~effect_id ~symbol ~location ~value_json
    ~provider_module ~provider_source_hash ~provider_artifact_path
    ~provider_phase_index ~derivation_source ~source_evidence_path =
  let singleton_int =
    match value_json with
    | `Int n -> `Int n
    | `String value -> (
        match parse_singleton_interval_value value with
        | Some n -> `Int n
        | None -> `Null)
    | _ -> `Null
  in
...
let make_external_summary ~export_name ~provider_module ~provider_source_hash
    ~provider_artifact_path ~return_node ~return_location ~return_value
    ~abstract_return_value ~provider_row ~provider_phase_index =
  let provenance =
    `Assoc
      [
        ("derivation_source", `String "provider-stage2-output");
  ...
  let return_effect_id =
    ScalarCall.provider_return_effect_id ~provider_module ~export_name
      ~return_location
  in
  let scalar_return =
    match
      ScalarCall.make_provider_return ~provider_module ~provider_source_hash
        ~provider_artifact_path ~export_name ~return_node ~return_location
        ~effect_id:return_effect_id ~provider_phase_index ~return_value
        ~abstract_return_value ~provider_row
    with
    | Ok scalar_return -> scalar_return
    | Error reason ->
        failwith
          ("invalid scalar return protocol for " ^ export_name ^ ": " ^ reason)
  in
  let return_effect = ScalarCall.return_effect_json scalar_return in
  let memory_delta_records =
    provider_row |> row_memory
    |> List.filter_map (fun cell ->
           ...
           if location = "" || location = return_location then None
             ...
             let summary_location =
               if domain_s = "pointer-memory-effect" then
                 "(" ^ export_name ^ ",p)"
               else location
             in
             let symbol =
               if domain_s = "pointer-memory-effect" then export_name
               else location
             in
             match
               MemoryDelta.make_provider_delta ~provider_module
                 ~provider_source_hash ~provider_artifact_path
                 ~provider_phase_index ~export_name ~domain
                 ~raw_location:location ~summary_location ~symbol
             ...
             | Error reason ->
                 failwith
                   ("invalid v3 memory delta for " ^ export_name ^ ": " ^ reason))
  in
    ...
    |> List.map (fun delta ->
           `Assoc
             [
               ...
               ( "memory_delta_schema",
                 `String MemoryDelta.memory_delta_schema_id );
               ("entries", `List (MemoryDelta.delta_chain_json delta));
             ])
  in
  let memory_effects =
    List.map MemoryDelta.compatibility_effect_json memory_delta_records
  in
  let global_effects =
    memory_effects
    |> List.filter (fun eff ->
           string_field "domain" eff = Some "global-write-read")
  in
  let pointer_effects =
    memory_effects
    |> List.filter (fun eff ->
           string_field "domain" eff = Some "pointer-memory-effect")
  in
  let taint_components =
    if contains export_name "taint" then
      [
        MemoryDelta.make_taint_component_json ~witness_id:"taint_product_pair"
          ~symbol:export_name ~location:return_location
    ...
    | [ taint_component ] ->
        [
          MemoryDelta.make_product_pair_evidence_json
            ~witness_id:"taint_product_pair" ~itv_value:abstract_return_value
            ~taint_component;
        ]
    | _ -> []
  in
  let v1_compat =
    {
      ...
      function_return_summary =
        ScalarCall.function_return_summary_json scalar_return;
      global_write_summary_placeholder =
        `Assoc
          [
                ...
                |> List.concat_map (fun exporter ->
                       ...
                       |> List.filter_map (fun export_decl ->
                              if export_decl.name = import_decl.name then
                                Some
                                  (importer, exporter, import_decl, export_decl)
                              else None))))
...
let ensure_supported_link_shape matches =
  let ambiguous_import_bindings =
    ...
    |> List.map (fun (importer, _, import_decl, _) ->
           importer.result.module_id ^ ":" ^ import_decl.kind ^ ":"
           ^ import_decl.name)
  in
...
let extract_semantic_exports matches provider_outputs : semantic_export list =
  ...
  |> List.map (fun (_, provider, _, export_decl) ->
         let provider_output =
           ...
           | None ->
               failwith
                 ("missing provider stage2 output: " ^ provider.result.module_id)
         in
         match
           find_return_row ~export_name:export_decl.name
             provider_output.StageT.final_output_table
         with
         | Some (row, abstract_return_value, return_value) ->
             let provider_phase_index = 1 in
             let external_summary =
               make_external_summary ~export_name:export_decl.name
                 ...
                 ~return_value ~abstract_return_value ~provider_row:row
         ...
         | None ->
             failwith
               ("provider stage2 output has no singleton return summary for "
              ^ export_decl.name))

let component_mentions_import import_name component =
  match
    assoc_field "semantic_expression" component.StageT.default_component
  with
...
let extern_root_for_import importer import_decl =
  ...
  let matching_roots =
    ...
    |> List.filter_map (fun component ->
           if component_mentions_import import_decl.name component then
             Some component.StageT.node
           else None)
    |> sort_strings
  in
  ...
  | [], [] ->
      failwith
        ("importer has no extern roots for linked obligation: "
       ^ importer.result.module_id)
  | [], roots ->
      failwith
        ("cannot resolve extern root for import " ^ import_decl.name ^ " in "
       ^ importer.result.module_id ^ ": " ^ comma_join roots)
  | roots, _ ->
      failwith
        ("ambiguous extern roots for import " ^ import_decl.name ^ " in "
       ^ importer.result.module_id ^ ": " ^ comma_join roots)

let linked_environment_for_matches matches
    (semantic_exports : semantic_export list) : linked_environment_entry list =
  matches
...
let scalar_return_of_semantic_export export =
  ...
  match
    ScalarCall.make_provider_return ~provider_module:export.provider_module
      ~provider_source_hash:export.provider_source_hash
      ~provider_artifact_path:export.provider_artifact_path
      ~export_name:export.export_name ~return_node:export.return_node
      ~return_location:export.return_location ~effect_id
  ...
  | Error reason ->
      failwith
        ("invalid scalar return protocol for " ^ export.export_name ^ ": "
       ^ reason)
...
let linked_stage2_input_for_importer importer linked_environment =
  let entries =
    ...
    |> List.filter (fun entry ->
           entry.importer_module = importer.result.module_id)
  in
  let linked_effects =
    entries
    |> List.map (fun entry ->
           ( entry.importer_extern_root,
             ( entry.linked_return_value,
               let scalar_return =
                 scalar_return_of_semantic_export entry.semantic_export
               in
               let linked_derivation =
                 ...
                 | Error reason ->
                     failwith
                       ("invalid linked scalar derivation for "
                      ^ entry.export_name ^ ": " ^ reason)
               in
               ScalarCall.add_fields
                 [
                   ("provider_module", `String entry.provider_module);
                   ( "provider_source_hash",
                     `String entry.semantic_export.provider_source_hash );
                   ( "provider_artifact_path",
                     `String entry.semantic_export.provider_artifact_path );
                   ("export_name", `String entry.export_name);
                   ( "return_location",
                     `String entry.semantic_export.return_location );
                   ("return_node", `String entry.semantic_export.return_node);
                   ( "abstract_return_value",
                     `String entry.semantic_export.abstract_return_value );
                   ( "provider_phase_index",
                   ...
                   ( "scalar_call_protocol_id",
                     `String (ScalarCall.scalar_protocol_id scalar_return) );
                   ("typed_scalar_metadata_valid", `Bool true);
                   ( "typed_scalar_linked_derivation",
                     ScalarCall.linked_derivation_metadata_json
                       linked_derivation );
                 ]
                 (`Assoc []) ) ))
  in
  {
    StageT.extern_effects =
      Stage2.make_linked_extern_effects ~source:importer.result.source
...
let linked_input_derivation_json linked_environment =
  ...
  |> List.map (fun entry ->
         ...
         let linked_derivation =
           ...
           | Error reason ->
               failwith
                 ("invalid linked scalar derivation for " ^ entry.export_name
                ^ ": " ^ reason)
         in
         ScalarCall.add_fields
           [
             ...
             ( "stage2_obligation",
               `String
                 "dynamic external/link fact derived from provider stage2 \
                  output" );
             ("derivation_source", `String "provider-stage2-output");
             ( "external_summary_schema",
               `String EffectSchema.schema_id );
             ...
             ("typed_scalar_metadata_valid", `Bool true);
             ( "typed_scalar_linked_derivation",
               ScalarCall.linked_derivation_metadata_json linked_derivation );
           ]
           (`Assoc []))

let shared_scc_cell ~kind ~module_id ~symbol ~location =
  StageT.make_residual_cell_id ~cell_table:"linked_scc"
...
let residual_scheduler_edges edges =
  ...
  |> List.map (fun (importer_id, provider_id, import_name, export_name) ->
         CycleScheduler.residual_call_binding_edge ~importer_module:importer_id
           ~provider_module:provider_id ~import_name ~export_name)

let residual_scheduler_for_edges ~scc_id ~group_ids ~edges =
  CycleScheduler.compute ~scc_id ~nodes:group_ids
...
let int_of_cell_json cell =
  ...
  | _ -> (
      match member "normalized_value" cell with
      | `Int n -> Some n
      | `String s -> int_of_trimmed s
      | _ -> None)
...
let shared_scc_result ~equation_id ~node ~location ~value ~kind ~source
    ~state_backed =
  {
    StageT.row =
      `Assoc
...
let shared_scc_constant_equation ~equation_id ~target_cell ~value ~kind ~source
    =
  StageT.make_residual_equation ~equation_id
    ~target_table:target_cell.StageT.cell_table
    ~target_node:target_cell.StageT.cell_node
    ~target_location:target_cell.StageT.cell_location ~dependencies:[]
    ~apply:(fun _state _input ->
      shared_scc_result ~equation_id ~node:target_cell.StageT.cell_node
        ~location:target_cell.StageT.cell_location ~value ~kind ~source
        ~state_backed:false)

let shared_scc_copy_equation ~equation_id ~target_cell ~source_cell ~kind
    ~source =
  StageT.make_residual_equation ~equation_id
    ~target_table:target_cell.StageT.cell_table
    ~target_node:target_cell.StageT.cell_node
    ...
    ~apply:(fun state _input ->
      let value =
        ...
        | None ->
            failwith
              ("shared SCC source cell missing before equation apply: "
             ^ equation_id ^ " depends on "
              ^ shared_scc_cell_key source_cell)
      in
      shared_scc_result ~equation_id ~node:target_cell.StageT.cell_node
        ~location:target_cell.StageT.cell_location ~value ~kind ~source
        ~state_backed:true)
         ...
         | Some (cell, value) ->
             ...
             let key =
               StageT.residual_cell_key
                 (StageT.make_residual_cell_id ~cell_table:"linked_scc"
                    ~cell_node:node ~cell_location:location)
             in
             Some (key, value, row))
...
let has_import_sink_observable ~importer_module ~import_name ~expected_value
    module_outputs =
  module_outputs
...
let observable_sink_cells ~entry module_outputs =
  ...
  |> List.filter_map (fun (module_id, _node, location, expression, value) ->
         ...
         then Some (location, value, expression)
         else None)
  |> List.sort_uniq compare

let shared_scc_edge_evidence_id ~importer_module ~provider_module ~import_name
    ~export_name =
  "shared-scc-edge:" ^ importer_module ^ ":" ^ import_name ^ "<-"
  ^ provider_module ^ ":" ^ export_name

let shared_scc_edge_json
  ...
  `Assoc
    [
      ...
      ( "stable_evidence_id",
        `String
          (shared_scc_edge_evidence_id ~importer_module ~provider_module
...
let shared_scc_value_provenance ~scc_id ~edge_kind ~stable_suffix =
  [
    ...
    ( "stable_evidence_id",
      `String ("shared-scc:" ^ scc_id ^ ":" ^ stable_suffix) );
  ]

let shared_scc_solver_report ~scc_id ~group_ids ~edges
    ~(final_exports : semantic_export list)
    ~(final_environment : linked_environment_entry list) ~module_outputs =
  let export_cell (export : semantic_export) =
    shared_scc_cell ~kind:"export-return" ~module_id:export.provider_module
      ~symbol:export.export_name ~location:export.return_location
  in
  let import_cell (entry : linked_environment_entry) =
    shared_scc_cell ~kind:"import-return" ~module_id:entry.importer_module
  ...
  let observable_cell (entry : linked_environment_entry) sink_location =
    shared_scc_cell ~kind:"import-observable" ~module_id:entry.importer_module
      ~symbol:entry.import_name ~location:sink_location
  in
  let export_equations =
    ...
    |> List.map (fun (export : semantic_export) ->
           shared_scc_constant_equation
             ~equation_id:
               ("0-export:" ^ export.provider_module ^ ":" ^ export.export_name)
             ~target_cell:(export_cell export) ~value:export.return_value
             ~kind:"export-return" ~source:"provider-stage2-output")
  in
           ...
           | Some export ->
               Some
                 (shared_scc_copy_equation
                    ~equation_id:
                      ("1-import:" ^ entry.importer_module ^ ":"
                     ^ entry.import_name ^ "<-" ^ entry.provider_module ^ ":"
                     ^ entry.export_name)
                    ~target_cell:(import_cell entry)
                    ~source_cell:(export_cell export) ~kind:"import-return"
  ...
  let observable_equations =
    ...
    |> List.concat_map (fun (entry : linked_environment_entry) ->
           ...
           |> List.map (fun (sink_location, value, _expression) ->
                  shared_scc_copy_equation
                    ~equation_id:
                      ("2-observable:" ^ entry.importer_module ^ ":"
  ...
  let equations =
    ...
    |> List.sort (fun left right ->
           compare left.StageT.equation_id right.StageT.equation_id)
  in
  let solved =
    Solver.solve
      ~input:{ StageT.extern_effects = `Assoc [] }
      ~static_rows:[] ~equations
  in
    ...
    |> List.map (fun export ->
           ...
           `Assoc
             ([
                ...
                ( "shared_scc_value_matches",
                  `Bool (solver_value = Some export.return_value) );
                ("derivation_source", `String "shared_scc_final_cells");
              ]
             @ shared_scc_value_provenance ~scc_id
                 ~edge_kind:"shared-scc-export-final-cell"
                 ~stable_suffix:
                   ("export:" ^ export.provider_module ^ ":"
                  ^ export.export_name)))
    |> sort_json
  in
  let imported_values =
    ...
    |> List.map (fun entry ->
           ...
           `Assoc
             ([
                ...
                ("source_shared_scc_cell_id", `String (shared_scc_cell_key cell));
                ( "shared_scc_value_matches",
                  `Bool (solver_value = Some entry.linked_return_value) );
                ("exact_singleton", `Bool true);
                ( "no_extra_imprecision",
                  `Bool (solver_value = Some entry.linked_return_value) );
                ("observable_values", `List sink_values);
                ( "observable_sink_dependency_present",
                  `Bool
                    (has_import_sink_observable
                       ~importer_module:entry.importer_module
                       ~import_name:entry.import_name
                       ~expected_value:entry.linked_return_value module_outputs)
                );
                ("derivation_source", `String "shared_scc_final_cells");
              ]
             @ shared_scc_value_provenance ~scc_id
                 ...
                 ~stable_suffix:
                   ("import:" ^ entry.importer_module ^ ":" ^ entry.import_name
  ...
  let imported_exact =
    ...
    |> List.for_all (fun value ->
           ...
           && List.for_all
                (fun obs ->
                  bool_field "shared_scc_value_matches" obs = Some true)
                (list_field "observable_values" value))
  in
  let exports_exact =
    accepted_exports
  ...
  `Assoc
    [
      ...
      ( "worklist_drained",
        `Bool (bool_field "worklist_drained" solver_log = Some true) );
      ...
      ( "shared_scc_cell_ids",
        `List
          (final_cells
          |> List.map (fun cell -> member "shared_scc_cell_id" cell)) );
      ...
      ( "shared_scc_scheduler_edges",
        `List
          (List.map CycleScheduler.edge_to_json
             (CycleScheduler.edges scheduler)) );
      ...
      ( "shared_scc_final_cell_scheduler_bridge",
        `Assoc
          [
            ...
            ( "scheduler_edge_count",
              `Int (List.length (CycleScheduler.edges scheduler)) );
            ("shared_scc_final_cell_count", `Int (List.length final_cells));
            ( "final_values_derive_from_shared_scc_final_cells",
              `Bool (imported_exact && exports_exact) );
            ( "callgraph_backed_schedule_required_for_cycle_acceptance",
              `Bool true );
            ( "callgraph_backed_schedule_present",
              `Bool scheduler_callgraph_backed );
          ] );
...
let shared_scc_import_values report =
  ...
  |> List.filter_map (fun value ->
         ...
         | Some importer_module, Some import_name, Some n ->
             Some
               ( (importer_module, import_name),
                 n,
                 string_field "source_shared_scc_cell_id" value )
         | _ -> None)

let environment_from_shared_scc report
    (environment : linked_environment_entry list) =
  ...
  |> List.map (fun (entry : linked_environment_entry) ->
         match
           values
           |> List.find_map
                (fun ((importer_module, import_name), value, source_cell) ->
                  if
                    importer_module = entry.importer_module
                    && import_name = entry.import_name
                  then Some (value, source_cell)
                  else None)
         with
         | None ->
             failwith
               ("shared SCC solver did not derive import cell: "
              ^ entry.importer_module ^ ":" ^ entry.import_name)
         | Some (value, _source_cell) ->
             if value <> entry.linked_return_value then
               failwith
                 ("shared SCC import value disagrees with provider export: "
                ^ entry.importer_module ^ ":" ^ entry.import_name)
             else entry)

let derive_linked_run_evidence ~linked_execute_returned ~modules ~per_module
    ~final_input_table ~final_output_table ~matched ~unresolved ~cycle_scc_count
  ...
  let executed_module_keys =
    List.map (fun (bundle, _) -> module_key bundle) per_module
  in
  ...
  let linked_residual_analyzer_ran =
    ...
    && obligations_closed && no_shortcut_path && cycle_worklist_drained
...
let linked_run_evidence_to_json evidence =
  `Assoc
    [
      ...
      ( "linked_cycle_worklist_drained",
        `Bool evidence.linked_cycle_worklist_drained );
      ( "linked_cycle_obligations_closed",
        `Bool evidence.linked_cycle_obligations_closed );
    ]
...
let execute_modules modules inputs =
  ...
  let phase_event phase_index module_id event =
    { phase_index; module_id; event }
  in
  let module_id bundle = bundle.result.module_id in
  let module_has_function_import bundle =
    function_matches
    |> List.exists (fun (importer, _, _, _) ->
  ...
  let module_import_matches bundle =
    function_matches
    |> List.filter (fun (importer, _, _, _) ->
           importer.result.module_id = bundle.result.module_id)
  in
  let module_export_matches bundle =
    function_matches
    |> List.filter (fun (_, provider, _, _) ->
           provider.result.module_id = bundle.result.module_id)
  in
  let remove_exports_for_modules module_ids exports =
    exports
    |> List.filter (fun (export : semantic_export) ->
           not (List.mem export.provider_module module_ids))
  in
  let dependency_edges_for module_ids =
    ...
    |> List.filter_map (fun (importer, provider, import_decl, export_decl) ->
           ...
           if List.mem importer_id module_ids && List.mem provider_id module_ids
           then
  ...
  let cyclic_group group =
    List.length group > 1
    || dependency_edges_for group
       |> List.exists (fun (from_id, to_id, _, _) -> from_id = to_id)
  in
  ...
  let scc_topology_json scc_id group =
    ...
    let scheduler =
      residual_scheduler_for_edges ~scc_id ~group_ids:group ~edges
    in
    ...
    `Assoc
      [
        ...
        ( "topology_kind",
          `String
            (if is_cyclic then "cyclic-import-export-scc"
             else "acyclic-singleton") );
        ("scheduler_source", `String (CycleScheduler.source scheduler));
        ( "callgraph_backed_scheduler",
          `Bool (CycleScheduler.callgraph_backed scheduler) );
        ( "scheduler_edges",
          `List (List.map CycleScheduler.edge_to_json scheduler_edges) );
        ( "scheduler_sccs",
          match CycleScheduler.to_json scheduler with
          | `Assoc fields -> (
              match List.assoc_opt "sccs" fields with
              | Some sccs -> sccs
              | None -> `List [])
          | _ -> `List [] );
      ]
  ...
  in
  let run_module ?environment_override ~phase_index ~semantic_exports
      ~linked_inputs bundle =
    ...
    let module_environment =
      ...
      | _ when linked_inputs && is_importer ->
          linked_environment_for_matches import_matches semantic_exports
      | _ -> []
    in
    let input =
      if linked_inputs && is_importer then
        linked_stage2_input_for_importer bundle module_environment
    ...
    let execution_event =
      if linked_inputs && is_importer then
        "importer-stage2-executed-with-linked-environment"
    ...
    let execution_events =
      [ phase_event execution_phase module_id execution_event ]
    in
    let new_exports =
      extract_semantic_exports export_matches [ (bundle, output) ]
    in
    let new_exports =
      new_exports
      |> List.map (fun (export : semantic_export) ->
             let external_summary =
               make_external_summary ~export_name:export.export_name
    ...
    in
    let next_phase =
      execution_phase + 1 + if export_events = [] then 0 else 1
    in
    ( next_phase,
      (bundle, output),
      new_exports,
      module_environment,
      env_events @ execution_events @ export_events )
  in
  let execute_acyclic_ready phase_index executed_ids per_module semantic_exports
      linked_environment phase_log ready =
    ready
    |> List.fold_left
         (fun ( phase_index,
                executed_ids,
           ...
           let ( next_phase,
                 ...
                 module_environment,
                 events ) =
             run_module ~phase_index ~semantic_exports ~linked_inputs:true
               bundle
           in
           ( next_phase,
         ...
         ( phase_index,
           ...
           phase_log )
  in
  let execute_cyclic_scc phase_index executed_ids per_module semantic_exports
      linked_environment phase_log group_bundles scc_id =
      ...
      |> List.fold_left
           (fun (phase_index, outputs, exports, environments, events) bundle ->
             let ( next_phase,
                   ...
                   module_environment,
                   module_events ) =
               run_module ~phase_index ~semantic_exports:base_exports
                 ~linked_inputs:false bundle
             in
             ( next_phase,
           ...
           (phase_index, [], [], [], [])
    in
    let round_snapshot ~round ~origin ~changed previous_exports current_exports
        environment =
      ...
      `Assoc
        [
          ...
          ( "bootstrap_binding_count",
            `Int
              (if origin = "bootstrap-unknown" then List.length current_exports
               else 0) );
          ( "provider_derived_binding_count",
            `Int
              (if origin = "provider-derived" then List.length current_exports
               else 0) );
          ( "previous_exports",
            `List (List.map semantic_export_to_json previous_exports) );
          ("exports", `List (List.map semantic_export_to_json current_exports));
          ( "linked_environment",
            `List (List.map (binding_json ~round ~origin) environment) );
        ]
    in
    let bootstrap_round =
    ...
    let bootstrap_environment =
      ...
      |> List.concat_map (fun bundle ->
             linked_environment_for_matches
               (module_import_matches bundle)
    ...
    in
    let import_solver =
      shared_scc_solver_report ~scc_id ~group_ids ~edges
        ~final_exports:bootstrap_exports
        ~final_environment:bootstrap_environment ~module_outputs:[]
    in
    let solver_environment =
      environment_from_shared_scc import_solver bootstrap_environment
    in
    let ( final_phase,
          final_outputs,
          _diagnostic_exports,
      ...
      |> List.fold_left
           (fun (phase_index, outputs, exports, environments, events) bundle ->
             let ( next_phase,
                   ...
                   module_environment,
                   module_events ) =
               run_module ~environment_override:solver_environment ~phase_index
                 ~semantic_exports:(base_exports @ bootstrap_exports)
                 ~linked_inputs:true bundle
             in
             ( next_phase,
               outputs @ [ module_output ],
               exports @ new_exports,
    ...
    let shared_scc_solver =
      let report =
        shared_scc_solver_report ~scc_id ~group_ids ~edges
          ~final_exports:bootstrap_exports ~final_environment:solver_environment
          ~module_outputs:
            (final_outputs |> List.concat_map module_output_dynamic_int_cells)
      in
      match report with
      | `Assoc fields ->
          `Assoc
            ([
               ...
               ( "final_linked_environment_source",
                 `String "shared_scc_final_cells" );
             ]
            @ fields)
      | other -> other
    in
    ...
    ( final_phase,
      ...
      cycle_report )
  in
  let rec schedule phase_index remaining executed_ids per_module
      semantic_exports linked_environment phase_log cycle_reports =
    ...
    | _ -> (
        ...
        if ready <> [] then
          let ( phase_index,
                ...
                linked_environment,
                phase_log ) =
            execute_acyclic_ready phase_index executed_ids per_module
              semantic_exports linked_environment phase_log ready
          in
          schedule phase_index blocked executed_ids per_module semantic_exports
            linked_environment phase_log cycle_reports
        else
          let groups = scc_groups remaining in
          let selected_group =
            groups
            |> List.find_opt (fun group ->
                   cyclic_group group && group_ready executed_ids group)
          in
          match selected_group with
          | None ->
              failwith
                "unsupported cyclic mixed importer/provider residual-linking \
                 topology"
          | Some group ->
              let group_bundles =
                remaining
              ...
              let rest =
                ...
                |> List.filter (fun bundle ->
                       not (List.mem (module_id bundle) group))
              in
              let scc_id =
                "linked-cycle-scc-"
                ^ string_of_int (List.length cycle_reports + 1)
              in
              let ( phase_index,
                    executed_ids,
                    per_module,
                    ...
                    phase_log,
                    cycle_report ) =
                execute_cyclic_scc phase_index executed_ids per_module
                  semantic_exports linked_environment phase_log group_bundles
                  scc_id
              in
              schedule phase_index rest executed_ids per_module semantic_exports
                linked_environment phase_log
  ...
  let final_input_table =
    ...
    |> List.concat_map (fun (bundle, output) ->
           output.StageT.final_input_table
           |> List.map (namespace_row bundle.result.module_id))
    |> sort_json
  in
  let final_output_table =
    per_module
    |> List.concat_map (fun (bundle, output) ->
           output.StageT.final_output_table
           |> List.map (namespace_row bundle.result.module_id))
    |> sort_json
  in
  let shape_witnesses =
    ...
    |> List.concat_map (fun (bundle, (output : StageT.stage2_output)) ->
           output.StageT.shape_witnesses
           |> List.map (fun witness ->
                  `Assoc
                    [
                      ("linked_module_id", `String bundle.result.module_id);
                      ("witness", witness);
                    ]))
    |> sort_json
  in
  ...
  let linked_cycle_iteration_count =
    ...
    |> List.fold_left
         (fun acc report ->
           max acc
             (match int_field "iteration_count" report with
             | Some n -> n
             | None -> 0))
         0
  in
  let linked_cycle_worklist_drained =
    cycle_reports
    |> List.for_all (fun report ->
           bool_field "worklist_drained" report = Some true)
  in
  let linked_cycle_obligations_closed =
  ...
  let linked_cycle_bootstrap_bindings_remaining =
    ...
    |> List.fold_left
         (fun acc report ->
           acc
           +
           match int_field "bootstrap_bindings_remaining" report with
           | Some n -> n
           | None -> 0)
         0
  in
  let linked_cycle_changed_bindings =
    ...
    |> List.concat_map (fun report ->
           ...
           |> List.map (fun round ->
                  match int_field "changed_binding_count" round with
                  | Some n -> `Int n
                  | None -> `Int 0))
  in
  ...
  let linked_cycle_shared_scc_worklist_run =
    ...
    || linked_cycle_shared_scc_solvers <> []
       && List.for_all
            (fun solver ->
              bool_field "shared_scc_worklist_run" solver = Some true)
            linked_cycle_shared_scc_solvers
  in
  ...
  let linked_cycle_callgraph_backed_schedule =
    ...
    || linked_cycle_schedulers <> []
       ...
       && List.for_all
            (fun report ->
              bool_field "callgraph_backed_schedule" report = Some true)
            linked_cycle_schedulers
       && List.for_all
            (fun report ->
              bool_field "callgraph_backed_schedule" report = Some true)
  ...
  let linked_cycle_shared_scc_state_read_count =
    ...
    |> List.fold_left
         (fun acc solver ->
           acc
           +
           match int_field "shared_scc_state_read_count" solver with
           | Some n -> n
           | None -> 0)
         0
  in
  let linked_cycle_shared_scc_equation_ids =
    linked_cycle_shared_scc_solvers
    |> List.concat_map (fun solver ->
           list_field "shared_scc_equation_ids" solver)
    |> sort_json
  in
  ...
  let linked_cycle_shared_scc_dependencies =
    linked_cycle_shared_scc_solvers
    |> List.concat_map (fun solver ->
           list_field "shared_scc_dependencies" solver)
    |> sort_json
  in
  let linked_cycle_shared_scc_worklist_schedule =
    linked_cycle_shared_scc_solvers
    |> List.concat_map (fun solver ->
           list_field "shared_scc_worklist_schedule" solver)
    |> sort_json
  in
  let linked_cycle_shared_scc_final_cells =
    linked_cycle_shared_scc_solvers
    |> List.concat_map (fun solver ->
           list_field "shared_scc_final_cells" solver)
    |> sort_json
  in
  let linked_cycle_imported_observable_values =
    linked_cycle_shared_scc_solvers
    |> List.concat_map (fun solver ->
           list_field "imported_cyclic_observable_values" solver)
    |> sort_json
  in
  let linked_cycle_accepted_exports =
    linked_cycle_shared_scc_solvers
    |> List.concat_map (fun solver ->
           list_field "linked_cycle_accepted_exports" solver)
    |> sort_json
  in
  let linked_cycle_exact_singleton_parity =
    ...
    || linked_cycle_imported_observable_values <> []
       && List.for_all
            (fun solver ->
              bool_field "cyclic_imported_value_exact_singleton_parity" solver
              = Some true
              && bool_field "cyclic_export_final_cell_parity" solver = Some true
              && bool_field "final_values_derive_from_shared_scc_final_cells"
                   solver
                 = Some true)
            linked_cycle_shared_scc_solvers
  in
  let linked_residual_analyzer_evidence =
    derive_linked_run_evidence ~linked_execute_returned:true ~modules
      ~per_module ~final_input_table ~final_output_table ~matched ~unresolved
      ~cycle_scc_count:linked_cycle_scc_count
      ~cycle_worklist_drained:
        (linked_cycle_worklist_drained && linked_cycle_shared_scc_worklist_run
       && linked_cycle_exact_singleton_parity
       && linked_cycle_callgraph_backed_schedule)
  in
  let module_logs =
    per_module
    |> List.map (fun (bundle, output) ->
           let dispatch =
             if module_has_function_import bundle then "linked-environment"
             else "per-module"
           in
           `Assoc
             [
               ...
               ( "final_input_row_count",
                 `Int (List.length output.StageT.final_input_table) );
               ( "final_output_row_count",
                 `Int (List.length output.StageT.final_output_table) );
  ...
  let linked_residual_solver_run =
    ...
    && List.for_all
         (fun log -> bool_field "residual_solver_run" log = Some true)
         module_execution_logs
  in
  let linked_worklist_drained =
    module_execution_logs <> []
    && List.for_all
         (fun log -> bool_field "worklist_drained" log = Some true)
         module_execution_logs
  in
  let linked_residual_equation_count =
    ...
    |> List.fold_left
         (fun acc log ->
           acc
           +
           match int_field "residual_equation_count" log with
           | Some n -> n
           | None -> 0)
         0
  in
  let linked_solver_iteration_count =
    ...
    |> List.fold_left
         (fun acc log ->
           max acc
             (match int_field "solver_iteration_count" log with
             | Some n -> n
             | None -> 0))
         0
  in
  let linked_changed_cell_count =
    ...
    |> List.fold_left
         (fun acc log ->
           acc
           +
           match int_field "changed_cell_count" log with
           | Some n -> n
           | None -> 0)
         0
  in
  let linked_state_read_count =
    ...
    |> List.fold_left
         (fun acc log ->
           acc
           +
           match int_field "state_read_count" log with Some n -> n | None -> 0)
         0
  in
  let linked_seed_input_read_count =
    ...
    |> List.fold_left
         (fun acc log ->
           acc
           +
           match int_field "seed_input_read_count" log with
           | Some n -> n
           | None -> 0)
         0
  in
  let linked_exact_cell_dependencies =
    ...
    |> List.concat_map (fun log ->
           ...
           | _ -> [])
  in
  let linked_stage2_input_derivation =
    linked_input_derivation_json linked_environment
  in
  let global_residual_fixpoint =
    GlobalResidualFixpoint.run ~linked_id:"post-link-linked-residual-analyzer"
      ~module_ids:(List.map (fun bundle -> bundle.result.module_id) modules)
      ~final_input_table ~final_output_table
      ...
      ~linked_cycle_worklist_schedule:linked_cycle_shared_scc_worklist_schedule
      ~linked_cycle_final_cells:linked_cycle_shared_scc_final_cells
  in
  {
    final_input_table;
    final_output_table;
      ...
      `Assoc
        [
          ...
          ( "global_sparse_fixpoint_component",
            global_residual_fixpoint
            |> member "global_sparse_fixpoint_component" );
          ...
          ( "global_residual_changed_cell_count",
            global_residual_fixpoint
            |> member "global_residual_changed_cell_count" );
          ( "global_residual_state_read_count",
            global_residual_fixpoint
            |> member "global_residual_state_read_count" );
          ...
          ( "global_residual_worklist_drained",
            global_residual_fixpoint
            |> member "global_residual_worklist_drained" );
          ...
          ( "global_residual_dependency_edges",
            global_residual_fixpoint
            |> member "global_residual_dependency_edges" );
          ...
          ( "global_residual_worklist_schedule",
            global_residual_fixpoint
            |> member "global_residual_worklist_schedule" );
          ...
          ( "linked_residual_analyzer_ran",
            `Bool linked_residual_analyzer_evidence.linked_residual_analyzer_ran
          );
          ( "linked_residual_solver_run",
            `Bool
              (linked_residual_analyzer_evidence.linked_residual_analyzer_ran
             && linked_residual_solver_run) );
          ( "linked_solver_backed_residual_fixpoint",
            `Bool
              (linked_residual_analyzer_evidence.linked_residual_analyzer_ran
             && linked_residual_solver_run) );
          ...
          ( "linked_exact_cell_dependencies",
            `List linked_exact_cell_dependencies );
          ( "linked_equation_apply_reads_solver_state",
            `Bool (linked_state_read_count > 0) );
          ("linked_worklist_drained", `Bool linked_worklist_drained);
          ("linked_overlay_only", `Bool false);
          ( "linked_residual_analyzer_evidence",
            linked_run_evidence_to_json linked_residual_analyzer_evidence );
          ( "linked_cyclic_residual_solver_run",
            `Bool
              (linked_cycle_scc_count > 0
             && linked_residual_analyzer_evidence.linked_residual_analyzer_ran
             && linked_cycle_shared_scc_worklist_run
             && linked_cycle_callgraph_backed_schedule) );
          ...
          ( "linked_cycle_worklist_drained",
            `Bool
             ...
             && linked_cycle_callgraph_backed_schedule) );
          ( "linked_cycle_obligations_closed",
            `Bool linked_cycle_obligations_closed );
          ( "linked_cycle_topology",
            `List
              (List.map (fun report -> member "topology" report) cycle_reports)
          );
          ("linked_cycle_scheduler", `List linked_cycle_schedulers);
          ( "linked_cycle_shared_scc_scheduler",
            `List linked_cycle_shared_scc_schedulers );
          ( "linked_cycle_callgraph_backed_schedule",
            `Bool linked_cycle_callgraph_backed_schedule );
          ( "linked_cycle_rounds",
            `List
              (cycle_reports
              |> List.concat_map (fun report -> list_field "rounds" report)) );
          ("linked_cycle_changed_bindings", `List linked_cycle_changed_bindings);
          ( "linked_cycle_bootstrap_bindings_remaining",
            `Int linked_cycle_bootstrap_bindings_remaining );
          ( "linked_cycle_stable_exports",
            `Bool
              (linked_cycle_scc_count = 0
              || linked_cycle_worklist_drained
                 && linked_cycle_bootstrap_bindings_remaining = 0) );
          ( "linked_cycle_shared_scc_solvers",
            `List linked_cycle_shared_scc_solvers );
          ...
          ( "shared_scc_state_read_count",
            `Int linked_cycle_shared_scc_state_read_count );
          ( "shared_scc_worklist_schedule",
            `List linked_cycle_shared_scc_worklist_schedule );
          ...
          ( "imported_cyclic_observable_values",
            `List linked_cycle_imported_observable_values );
          ( "cyclic_imported_value_exact_singleton_parity",
            `Bool linked_cycle_exact_singleton_parity );
          ( "cycle_final_values_derive_from_shared_scc_final_cells",
            `Bool linked_cycle_exact_singleton_parity );
          ...
          ( "module_identity_set_matches",
            `Bool linked_residual_analyzer_evidence.module_identity_set_matches
          );
          ("per_module_stage2_inputs_used", `Bool true);
          ( "provider_derived_importer_inputs_used",
            `Bool (linked_environment <> []) );
          ( "semantic_exports",
            `List (List.map semantic_export_to_json semantic_exports) );
          ( "external_summaries",
            `List
              (List.map
                 (fun export ->
                   external_summary_to_json export.external_summary)
                 semantic_exports) );
          ( "linked_environment",
            `List (List.map linked_environment_entry_to_json linked_environment)
          );
          ( "linked_stage2_input_derivation",
            `List (linked_input_derivation_json linked_environment) );
          ("phase_log", `List (List.map phase_event_to_json phase_log));
          ("namespacing_strategy", `String "linked_module_id-prefix");
          ( "linked_residual_row_count",
            `Int linked_residual_analyzer_evidence.linked_residual_row_count );
          ( "residual_rows_observed",
            `Bool linked_residual_analyzer_evidence.residual_rows_observed );
          ("matched_obligation_count", `Int (List.length matched));
          ("unresolved_obligation_count", `Int (List.length unresolved));
          ( "obligations_closed",
            `Bool linked_residual_analyzer_evidence.obligations_closed );
          ( "no_shortcut_path",
            `Bool linked_residual_analyzer_evidence.no_shortcut_path );
          ("matched_obligations", `List matched);
          ("unresolved_obligations", `List unresolved);
          ("module_logs", `List module_logs);
          ( "non_equivalence_claim",
            `String
              "scoped emitted-equation/witness-universe soundness plus fixture \
               parity; not arbitrary-C whole-program theorem" );
        ];
  }

let make_analyzer ~linked_id modules =
  if List.length modules < 2 then
    failwith "residual linking requires at least two module bundles";
  { linked_id; modules; run = execute_modules modules }
...
let output_to_json output =
  `Assoc
    [
      ...
      ( "semantic_exports",
        `List (List.map semantic_export_to_json output.semantic_exports) );
      ( "external_summaries",
        `List
          (List.map
             (fun export -> external_summary_to_json export.external_summary)
             output.semantic_exports) );
      ( "linked_environment",
        `List
          (List.map linked_environment_entry_to_json output.linked_environment)
      );
      ( "linked_stage2_input_derivation",
...
let artifact_json ~doc_path analyzer output =
  ...
  `Assoc
    [
      ...
      ( "claim",
        `String
          "PE(I,m1), PE(I,m2), ... -> residual linking -> linked residual \
           analyzer" );
      ( "validation_oracle",
        `String
          "typed bundle execution plus parsed-CIL structural obligation \
           matching" );
      ...
      ( "declared_imports",
        `List
          (modules
          |> List.concat_map (fun bundle ->
                 bundle.declared_imports
                 |> List.map (fun d ->
                        `Assoc
                          [
                            ("module_id", `String bundle.result.module_id);
                            ("declaration", declaration_to_json d);
                          ]))
          |> sort_json) );
      ( "declared_exports",
        `List
            ...
            ( "keys",
              `List
                ...
                |> List.map (fun bundle ->
                       `String
                         (bundle.result.module_id ^ ":"
                        ^ bundle.result.source_hash))) );
            ("derivation_source", `String "provider-stage2-output");
            ( "linked_environment_generated",
              `Bool (output.linked_environment <> []) );
          ] );
      ( "semantic_exports",
        `List (List.map semantic_export_to_json output.semantic_exports) );
      ( "external_summaries",
      ...
      ( "linked_residual_analyzer_ran",
        `Bool
          output.linked_residual_analyzer_evidence.linked_residual_analyzer_ran
      );
      ( "linked_residual_solver_run",
        output.execution_log |> member "linked_residual_solver_run" );
      ( "linked_solver_backed_residual_fixpoint",
        output.execution_log |> member "linked_solver_backed_residual_fixpoint"
      );
      ( "linked_solver_iteration_count",
        output.execution_log |> member "linked_solver_iteration_count" );
      ( "linked_changed_cell_count",
        output.execution_log |> member "linked_changed_cell_count" );
      ( "linked_residual_equation_count",
        output.execution_log |> member "linked_residual_equation_count" );
      ( "linked_state_read_count",
        output.execution_log |> member "linked_state_read_count" );
      ( "linked_seed_input_read_count",
        output.execution_log |> member "linked_seed_input_read_count" );
      ( "linked_exact_cell_dependencies",
        output.execution_log |> member "linked_exact_cell_dependencies" );
      ( "linked_equation_apply_reads_solver_state",
        output.execution_log
        |> member "linked_equation_apply_reads_solver_state" );
      ( "linked_worklist_drained",
        output.execution_log |> member "linked_worklist_drained" );
      ( "linked_overlay_only",
        output.execution_log |> member "linked_overlay_only" );
      ( "linked_cyclic_residual_solver_run",
        output.execution_log |> member "linked_cyclic_residual_solver_run" );
      ( "linked_cycle_scc_count",
        output.execution_log |> member "linked_cycle_scc_count" );
      ( "linked_cycle_iteration_count",
        output.execution_log |> member "linked_cycle_iteration_count" );
      ( "linked_cycle_worklist_drained",
        output.execution_log |> member "linked_cycle_worklist_drained" );
      ( "linked_cycle_obligations_closed",
        output.execution_log |> member "linked_cycle_obligations_closed" );
      ( "linked_cycle_topology",
        output.execution_log |> member "linked_cycle_topology" );
      ( "linked_cycle_scheduler",
        output.execution_log |> member "linked_cycle_scheduler" );
      ( "linked_cycle_shared_scc_scheduler",
        output.execution_log |> member "linked_cycle_shared_scc_scheduler" );
      ( "linked_cycle_callgraph_backed_schedule",
        output.execution_log |> member "linked_cycle_callgraph_backed_schedule"
      );
      ( "linked_cycle_rounds",
        output.execution_log |> member "linked_cycle_rounds" );
      ( "linked_cycle_changed_bindings",
        output.execution_log |> member "linked_cycle_changed_bindings" );
      ( "linked_cycle_bootstrap_bindings_remaining",
        output.execution_log
        |> member "linked_cycle_bootstrap_bindings_remaining" );
      ( "linked_cycle_stable_exports",
        output.execution_log |> member "linked_cycle_stable_exports" );
      ( "linked_cycle_shared_scc_solvers",
        output.execution_log |> member "linked_cycle_shared_scc_solvers" );
      ( "shared_scc_worklist_run",
        output.execution_log |> member "shared_scc_worklist_run" );
      ( "shared_scc_equation_ids",
        output.execution_log |> member "shared_scc_equation_ids" );
      ( "shared_scc_cell_ids",
        output.execution_log |> member "shared_scc_cell_ids" );
      ( "shared_scc_dependencies",
        output.execution_log |> member "shared_scc_dependencies" );
      ( "shared_scc_state_read_count",
        output.execution_log |> member "shared_scc_state_read_count" );
      ( "shared_scc_worklist_schedule",
        output.execution_log |> member "shared_scc_worklist_schedule" );
      ( "shared_scc_final_cells",
        output.execution_log |> member "shared_scc_final_cells" );
      ( "linked_cycle_accepted_exports",
        output.execution_log |> member "linked_cycle_accepted_exports" );
      ( "imported_cyclic_observable_values",
        output.execution_log |> member "imported_cyclic_observable_values" );
      ( "cyclic_imported_value_exact_singleton_parity",
        output.execution_log
        |> member "cyclic_imported_value_exact_singleton_parity" );
      ( "cycle_final_values_derive_from_shared_scc_final_cells",
        output.execution_log
        |> member "cycle_final_values_derive_from_shared_scc_final_cells" );
      ( "linked_residual_analyzer_evidence",
        linked_run_evidence_to_json output.linked_residual_analyzer_evidence );
      ("per_module_stage2_inputs_used", `Bool true);
      ( "provider_derived_importer_inputs_used",
      ...
      ("namespacing_strategy", `String "linked_module_id-prefix");
      ( "forbidden_prelink_entrypoints",
        `List [ `String forbidden_global_entry; `String forbidden_merge_entry ]
      );
...
let write_artifact ~path ~doc_path analyzer output =
  Real_sparrow_artifact.write_json path
    (artifact_json ~doc_path analyzer output)
